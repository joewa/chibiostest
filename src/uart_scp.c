...
static void rxend(UARTDriver *uartp) {
  ...
  if (crc == rxBuffer[7]) {
    ...
    case SCP_LEDRED:
      if (rxBuffer[1] == 0x00) {
        palClearPad(GPIOE, GPIOE_LED3_RED);         //STM32F3Discovery
        //palClearPad(GPIOB, GPIOB_LEDR);       //Strip v2
        txBuffer[0] = SCP_ACK;
      }
      else if (rxBuffer[1] == 0xFF) {
        palSetPad(GPIOE, GPIOE_LED3_RED);           //STM32F0Discovery
    ...
    case SCP_LEDGREEN:
      if (rxBuffer[1] == 0x00) {
        palClearPad(GPIOE, GPIOE_LED7_GREEN);         //STM32F3Discovery
        //palClearPad(GPIOB, GPIOB_LEDG);       //Strip v2
        txBuffer[0] = SCP_ACK;
      }
      else if (rxBuffer[1] == 0xFF) {
        palSetPad(GPIOE, GPIOE_LED7_GREEN);           //STM32F0Discovery
...
/*static UARTConfig uart_cfg_1 = {txend1, txend2, rxend, rxchar, rxerr, 9600, 0,
                                USART_CR2_LINEN, 0};*/
static UARTConfig uart_cfg_1 = {txend1, txend2, rxend, rxchar, rxerr};
